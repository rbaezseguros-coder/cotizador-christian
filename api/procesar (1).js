...
- TODAS las compañías (Norte, FedPat, Sancor): si el PDF muestra el monto de Responsabilidad Civil, reemplazar "Responsabilidad Civil" en el array cubre por "Responsabilidad Civil $XXX.XXX.XXX" con el monto extraído del PDF (sin decimales, con puntos de miles). Ej: "$520.000.000,00" → "Responsabilidad Civil $520.000.000". Para FedPat buscar "Limite Resp. Civil $", para Norte buscar "Responsabilidad Civil $", para Sancor buscar el monto en la tabla de coberturas.
...
function buildPrecioLinea(cob, E, modoNorte = 'cuotas', modoFedpat = 'cuotas') {
  ...
  if (comp === 'norte') {
    if (esSoloTarjeta) {
      ...
      return cuatri ? `${E.tarjeta} *Pago mensual: ${cuatri}* (solo mediante Débito Automático con Tarjeta de Crédito)\n` : '';
    }
    if (modoNorte === 'contado') {
      return contado ? `${E.dinero} *Pago único: ${contado}*\n` : '';
    }
  ...
  if (comp === 'fedpat') {
    if (esSoloTarjeta) {
      ...
      return semest ? `${E.tarjeta} *Pago mensual: ${semest}* (solo mediante Débito Automático con Tarjeta de Crédito)\n` : '';
    }
    if (modoFedpat === 'contado') {
      return contado ? `${E.dinero} *Pago único: ${contado}*\n` : '';
    }
